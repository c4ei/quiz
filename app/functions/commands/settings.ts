...
const settings = async (): Promise<void> => {
	...
	bot.callbackQuery("settings_languages", async (ctx) => {
		...
		buttons.text(translate(lang.language, "settings_command_language_korean"), "settings_set_korean");
		buttons.row();
		// buttons.url(
	...
	bot.callbackQuery("settings_set_korean", async (ctx) => {
		const lang = await telegram.api.message.getLanguage(ctx);

		if (lang.group_id !== 0) {
			await db.settings.update(
				{ group_id: telegram.api.message.getChatID(ctx) },
				{ group_id: telegram.api.message.getChatID(ctx), language: "ko", pin_message: lang.pin_message },
			);
		} else {
			await db.settings.add({
				group_id: telegram.api.message.getChatID(ctx),
		...
		await telegram.api.message.send(
			ctx,
			telegram.api.message.getChatID(ctx),
			translate(lang.language, "settings_command_current_korean"),
		);
	});
};
